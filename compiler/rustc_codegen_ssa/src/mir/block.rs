...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
            ...
            self.get_discriminant_switch_block_parts(statements, terminator)
        {
            ...
            match enum_layout.layout.variants {
                _ if enum_layout.abi.is_uninhabited() => {
                    debug!("optimized uninhabited switch to unreachable");
                ...
                abi::Variants::Multiple { ref variants, ref tag_encoding, .. } => {
                    fn filter_switch_targets(
                        ...
                        let first_unreachable = targets.iter().position(|(val, _)| !f(val))?;

                        Some(mir::SwitchTargets::new(
                            targets.iter().take(first_unreachable).chain(
                                targets
                                    .iter()
                                    .skip(first_unreachable + 1)
                                    .filter(|(val, _)| !f(*val)),
                            ),
                            targets.otherwise(),
                        ))
                    ...
                    if let Some(filtered_switch_targets) = filtered_switch_targets {
                        debug!(
                            "pruned discriminant switch targets (removed {} out of {} targets)",
                            targets.iter().len(),
    ...
    fn get_discriminant_switch_block_parts<'t>(
        ...
        if !self.cx.tcx().sess.opts.debugging_opts.opt_monomorphized_switch {
            return None;
        }
